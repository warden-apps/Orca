...
const state = { ready:false, db:null, sample:false, period:"", config:null, plan:null, months:{},
  filterCat:"", filterAcc:"", search:"", scope:"period", userMovedPeriod:false };
...
function migrate() {
  ...
  Object.values(state.months).forEach(m => {
    ...
    if (!m.incomeAmt || typeof m.incomeAmt !== "object") m.incomeAmt = {};
    m.tx.forEach(t => {
...
/* ---- undo / redo ----------------------------------------------------------
   Undo lives at the storage layer: every write compares against what was last
   saved for that document and remembers the previous version. Writes made in
   the same tick become one step, so "delete this entry" or "reset everything"
   undo as a single action however many documents they touch. Nothing at the
   call sites has to remember to record anything. */
const lastSaved = new Map();          // path -> JSON last written or loaded
const undoStack = [], redoStack = [];
let undoGroup = null, undoLabel = "", restoring = false;

/* data arriving from storage or another device: the new baseline, not a change */
function markSaved(path, obj) { lastSaved.set(path, obj == null ? null : JSON.stringify(obj)); }
/* name the next undo step; call just before a change */
function willChange(label) { undoLabel = label; }

function recordBefore(path, before) {
  if (!undoGroup) {
    undoGroup = { label: undoLabel || "last change", before: new Map() };
    undoLabel = "";
    redoStack.length = 0;           // a fresh change forks history
    setTimeout(flushUndoGroup, 0);
  }
  if (!undoGroup.before.has(path)) undoGroup.before.set(path, before);
}
function flushUndoGroup() {
  if (!undoGroup) return;
...
function docFor(path) {
  if (path === "config/main") return state.config;
  if (path === "plan/main") return state.plan;
  return state.months[path.slice(7)];
}
function applyStep(step, pushInverseTo) {
  const inverse = { label: step.label, before: new Map() };
  step.before.forEach((_, path) => inverse.before.set(path, lastSaved.has(path) ? lastSaved.get(path) : null));
  restoring = true;
  step.before.forEach((json, path) => {
    const val = json ? JSON.parse(json) : null;
    if (path === "config/main") state.config = val || blankConfig();
    else if (path === "plan/main") state.plan = val || blankPlan(state.config.currency);
    else { const key = path.slice(7); state.months[key] = val || { key, tx: [], skip: [], paid: [], gotIncome: [], billAmt: {}, incomeAmt: {} }; }
    persist(path, docFor(path), 0);
  });
  restoring = false;
...
function undo() {
  ...
  applyStep(step, redoStack);
  toast("Undid: " + step.label, "Redo", redo);
}
function redo() {
  flushUndoGroup();
  const step = redoStack.pop();
  if (!step) { toast("Nothing to redo"); return; }
  applyStep(step, undoStack);
  toast("Redid: " + step.label, "Undo", undo);
}
function renderUndo() {
  const b = document.getElementById("btnUndo");
  if (!b) return;
  const top = undoGroup || undoStack[undoStack.length - 1];
  b.disabled = !top;
  ...
  if (r) {
    ...
    r.disabled = !next;
    r.textContent = next ? "Redo: " + next.label : "Redo";
  }
}
...
function persist(path, obj, delay) {
  ...
  if (!restoring) {
    const prev = lastSaved.has(path) ? lastSaved.get(path) : null;
    if (prev !== now) recordBefore(path, prev);
  }
...
function monthDoc(key) {
  if (!state.months[key]) state.months[key] = { key, tx: [], skip: [], paid: [], gotIncome: [], billAmt: {}, incomeAmt: {} };
  ...
  if (!m.incomeAmt) m.incomeAmt = {};
  return m;
}
/* the shape a month doc takes when it arrives from storage */
const monthFrom = (key, v) => ({ key, tx: v.tx || [], skip: v.skip || [], paid: v.paid || [],
  gotIncome: v.gotIncome || [], billAmt: v.billAmt || {}, incomeAmt: v.incomeAmt || {} });
...
/* pay for a given period: the figure you corrected it to, else the usual amount */
function incomeAmount(inc, key) {
  const m = state.months[key];
  const v = m && m.incomeAmt ? m.incomeAmt[inc.id] : undefined;
  return typeof v === "number" ? v : num(inc.amount);
}
...
function derive(key) {
  ...
  const baseIncome = (plan.incomes || []).reduce((s, i) => s + incomeAmount(i, key), 0);
  ...
  const incomesDue = (plan.incomes || []).map(i => ({ ...i, amt: incomeAmount(i, key),
    changed: !!(m.incomeAmt && typeof m.incomeAmt[i.id] === "number"), got: got.has(i.id), due: dayInPeriod(i.day, key) }));
...
function balancesAsOf(cutoff) {
  ...
  Object.keys(state.months).forEach(key => {
    ...
    (state.plan.incomes || []).forEach(i => {
      ...
      if (bal[i.acc] != null) bal[i.acc] += incomeAmount(i, key);
    });
  });
  return bal;
}
...
function renderTodo(d) {
  ...
  d.incomesDue.forEach(i => {
    if (i.got || i.amt <= 0) return;
    items.push({ kind: "income", id: i.id, name: i.name, due: i.due, amt: i.amt,
  ...
  items.forEach(it => {
    ...
    const mid = el("div", "tmain");
    // the name opens the full editor: amount, paid, or skip this month
    const nameBtn = el("button", "tt tlink", it.name);
    nameBtn.type = "button";
    nameBtn.title = it.kind === "income" ? "Change this month's pay" : "Change, skip or mark paid for this month";
    nameBtn.addEventListener("click", () => {
      if (ensureReal()) return;
      if (it.kind === "income") openPayMonth(it.id, state.period); else openBillMonth(it.id, state.period);
    });
    mid.append(nameBtn);
    const sub = el("div", "tc");
    if (it.kind === "income") {
    ...
    // money you are owed is not "overdue" — that reads like you owe it
    if (overdue) sub.append(el("span", "tag late", it.kind === "income" ? "not in yet" : "overdue"));
    mid.append(sub);
    row.append(mid);
    ...
    if (it.kind === "bill" && it.variable) {
      ...
      const saveAmt = markPaid => {
        ...
        if (ensureReal()) return;
        willChange((markPaid ? "pay " : "set ") + it.name);
        ...
        done(it.name + (markPaid ? " paid" : " set") + (v != null ? " — " + money(v) : ""));
      };
...
function renderNetWorth(d) {
  ...
  const pendingInc = d.incomesDue.filter(i => !i.got).reduce((s, i) => s + i.amt, 0);
  const parts = [];
...
function renderLedger(d) {
  ...
  const allTime = state.scope === "all";
  const items = [];
  const matchTx = t => {
    if (state.filterCat && (t.t !== "exp" || t.c !== state.filterCat)) return false;
    if (state.filterAcc && !(t.acc === state.filterAcc || t.from === state.filterAcc || t.to === state.filterAcc)) return false;
    if (q && !((t.n || "") + " " + (t.t === "exp" ? catById(t.c).name : "") + " " + t.a).toLowerCase().includes(q)) return false;
    return true;
  };

  if (allTime) {
    // every period's entries, to track down a mistake without paging back month by month
    Object.keys(state.months).forEach(k => (state.months[k].tx || []).forEach(t => { if (matchTx(t)) items.push({ ...t, _key: k }); }));
  } else {
    (d.incomesDue || []).forEach(i => {
      if (i.amt <= 0 || state.filterCat) return;
      if (state.filterAcc && i.acc !== state.filterAcc) return;
      if (q && !(i.name + " pay salary " + i.amt).toLowerCase().includes(q)) return;
      items.push({ pay: true, id: i.id, d: iso(i.due), a: i.amt, n: i.name, acc: i.acc, got: i.got, changed: i.changed, t: "inc" });
    });
    d.bills.forEach(b => {
      if (state.filterCat && b.cat !== state.filterCat) return;
      if (state.filterAcc && b.acc !== state.filterAcc) return;
      if (q && !(b.name + " " + catById(b.cat).name + " " + b.amt).toLowerCase().includes(q)) return;
      items.push({ bill: true, id: b.id, d: iso(b.due), a: b.amt, c: b.cat, n: b.name, acc: b.acc,
        settled: b.settled, est: b.est, t: "exp" });
    });
    d.tx.forEach(t => { if (matchTx(t)) items.push({ ...t, _key: d.key }); });
  }
  items.sort((a, b) => (a.d < b.d ? 1 : a.d > b.d ? -1 : 0));

  const scopeNote = $("ledgerScope");
  if (scopeNote) scopeNote.textContent = allTime
    ? items.length + " match" + (items.length === 1 ? "" : "es") + " across every period · bills and pay are listed per period"
    : "Tap any row to change it";

  if (!items.length) {
    box.append(el("p", "empty", allTime ? "Nothing in any period matches that. Try part of a shop name, or an amount."
      : q || state.filterCat || state.filterAcc ? "Nothing matches that filter — switch to All time to search every period."
      : "No entries yet this period."));
    return;
  }
  const shown = allTime ? items.slice(0, 300) : items;

  const todayStr = iso(today());
  const dayTotals = new Map();
  shown.forEach(it => { if (it.t === "exp") dayTotals.set(it.d, (dayTotals.get(it.d) || 0) + num(it.a)); });
  let lastDay = null;
  ...
  shown.forEach(it => {
    if (it.d !== lastDay) {
      ...
      head.append(el("span", null, it.d === todayStr ? "Today · " + dd.toLocaleDateString(undefined, { day: "numeric", month: "short" })
        : dd.toLocaleDateString(undefined, allTime ? { weekday: "short", day: "numeric", month: "short", year: "numeric" }
          : { weekday: "short", day: "numeric", month: "short" })));
      head.append(el("span", "dt", dayTotals.get(it.d) ? money(dayTotals.get(it.d)) + " out" : "—"));
      box.append(head);
    }
    const row = el("div", "lrow tap");
    row.tabIndex = 0;
    ...
    const sub = el("div", "lc");
    if (it.pay) {
      sub.append(el("span", null, "Pay into " + accById(it.acc).name), el("span", "tag" + (it.got ? "" : " late"), it.got ? "received" : "not in yet"));
      if (it.changed) sub.append(el("span", "tag est", "changed this month"));
    } else if (it.t === "inc") {
    ...
    // tapping anywhere on a row opens the right editor for what it is
    const open = () => {
      if (ensureReal()) return;
      if (it.pay) openPayMonth(it.id, d.key);
      else if (it.bill) openBillMonth(it.id, d.key);
      else openEntry({ tx: it, key: it._key });
    };
    row.title = it.pay ? "Change this month's pay" : it.bill ? "Change, skip or mark paid for this month" : "Change or delete this entry";
    row.addEventListener("click", open);
    row.addEventListener("keydown", e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); open(); } });

    const acts = el("div", "lacts");
    if (it.bill || it.pay) {
      const on = it.bill ? it.settled : it.got;
      const b = el("button", "lx");
      b.innerHTML = on ? "☑" : "☐";
      b.title = it.bill ? (on ? "Mark as not yet paid" : "Mark as paid") : (on ? "Mark as not received" : "Mark as received");
      b.setAttribute("aria-label", b.title + " — " + it.n);
      b.addEventListener("click", e => {
        e.stopPropagation();
        if (ensureReal()) return;
        const m = monthDoc(d.key);
        const list = it.bill ? m.paid : m.gotIncome;
        const verb = it.bill ? (on ? "unpay " : "pay ") : (on ? "unreceive " : "receive ");
        willChange(verb + it.n);
        const i = list.indexOf(it.id);
        if (i >= 0) list.splice(i, 1); else list.push(it.id);
        saveMonth(d.key); renderAll();
        done(it.n + (it.bill ? (on ? " marked not paid" : " marked paid") : (on ? " marked not received" : " marked received")));
      });
      acts.append(b);
    } else {
      if (it.t === "exp") {
  ...
  if (allTime && items.length > shown.length) box.append(el("p", "empty", "Showing the newest 300 — narrow the search to find older ones."));
}
...
function ensureReal() {
  ...
  // a fresh book is the starting point, not a change there is anything to undo back to
  markAllSaved();
  restoring = true; saveConfig(); savePlan(); restoring = false;
  watchPeriod();
...
/* after any change: say what happened and offer to take it back */
function done(msg) { toast(msg, "Undo", undo); }

function deleteTx(key, id) {
  const m = monthDoc(key);
  const entry = m.tx.find(t => t.id === id);
  if (!entry) return;
  willChange("delete “" + (entry.n || "entry") + "”");
  m.tx = m.tx.filter(t => t.id !== id);
  saveMonth(key); renderAll();
  done("Deleted “" + (entry.n || "entry") + "”");
}
...
function repeatTx(tx) {
  ...
  willChange("log “" + (tx.n || "entry") + "” again");
  ...
  renderAll();
  done("Logged “" + (tx.n || "entry") + "” " + money(num(tx.a)) + " again for today");
}
...
function addQuick() {
  ...
  const m = monthDoc(key);
  const note = parsed.note || (parsed.income ? "Extra income" : catById(cat).name);
  willChange("add “" + note + "”");
  m.tx.push({ id: uid(), d: dateVal, a: parsed.amount, c: cat, acc, n: note, t: parsed.income ? "inc" : "exp" });
  saveMonth(key);
  if (acc !== state.config.lastAcc) { state.config.lastAcc = acc; saveConfig(); }
  $("qaText").value = ""; $("qaCat").value = "";
  if (key !== state.period) { state.userMovedPeriod = true; state.period = key; watchPeriod(); }
  renderAll();
  done("Added “" + note + "” " + money(parsed.amount));
  $("qaText").focus();
}
...
function openSheet(title, buildBody, buttons) {
  ...
  foot.textContent = "";
  (buttons || []).forEach(b => {
    const btn = el("button", "btn " + (b.primary ? "btn-solid" : b.danger ? "btn-danger-solid" : ""), b.label);
    btn.addEventListener("click", b.onClick); foot.append(btn);
  });
  $("scrim").hidden = false;
  document.body.style.overflow = "hidden";
  setTimeout(() => { const f = body.querySelector("input,select,textarea,button"); if (f) f.focus(); }, 30);
}
/* Sheets that edit the plan live (as you type) register a cancel that puts the
   last saved version back, so closing without saving really discards. */
let sheetCancel = null;
function closeSheet() {
  $("scrim").hidden = true; document.body.style.overflow = ""; sheetSaver = null;
  const cancel = sheetCancel; sheetCancel = null;
  if (cancel) { cancel(); renderAll(); }
}
/* committed = whatever storage last holds for the settings and plan */
function revertUnsaved() {
  const c = lastSaved.get("config/main"), p = lastSaved.get("plan/main");
    ...
    GROUPS.forEach(g => {
      ...
      state.plan.cats.filter(c => c.group === g.id).forEach(c => {
        ...
        x.addEventListener("click", () => {
          ...
          willChange("remove category “" + c.name + "”");
      ...
      add.addEventListener("click", () => {
        willChange("add a category");
        state.plan.cats.push({ id: uid(), name: "New category", group: g.id, budget: 0 }); savePlan(); openBudgets();
      });
      list.append(add); sec.append(list); body.append(sec);
    });
    update();
    sheetSaver = () => {
      willChange("change budgets");
      savePlan(); sheetCancel = null; closeSheet(); renderAll();
      done("Budgets saved");
    };
  }, [
...
function suggestBudgets() {
  ...
  willChange("suggest budgets");
  savePlan();
}

/* ---------- the entry sheet ----------
   One sheet adds or fixes any expense, income or transfer, so correcting a
   mistake from last week works exactly like logging something now. Categories
   and accounts are tap-chips rather than long dropdowns, with the categories
   you use most placed first. */

/* amounts as people type them: "1,200", "¥980", "12.50" */
const readAmount = s => { const v = parseFloat(String(s).replace(/[^\d.]/g, "")); return Number.isFinite(v) ? v : 0; };

function recentCats(limit) {
  const since = new Date(today()); since.setDate(since.getDate() - 90);
  const counts = new Map();
  Object.values(state.months).forEach(m => (m.tx || []).forEach(t => {
    if (t.t === "exp" && t.c && parseISO(t.d) >= since) counts.set(t.c, (counts.get(t.c) || 0) + 1);
  }));
  const ranked = Array.from(counts.entries()).sort((a, b) => b[1] - a[1]).map(e => e[0])
    .filter(id => state.plan.cats.some(c => c.id === id));
  // a new book has no history yet: start from the everyday spending most people log
  ["konbini", "grocery", "dining", "cafe", "transport", "household", "shopping", "fun"].forEach(id => {
    if (!ranked.includes(id) && state.plan.cats.some(c => c.id === id)) ranked.push(id);
  });
  return ranked.slice(0, limit);
}

/* a row of tap-to-pick chips; ._set(v) re-selects without redrawing */
function chipRow(items, value, onPick) {
  const wrap = el("div", "chips");
  const btns = items.map(it => {
    const b = el("button", "chip");
    b.type = "button";
    if (it.hue) { const s = el("span", "swatch"); s.style.background = it.hue; b.append(s); }
    b.append(document.createTextNode(it.label));
    b.addEventListener("click", () => { wrap._set(it.value); onPick(it.value); });
    wrap.append(b);
    return b;
  });
  wrap._set = v => btns.forEach((b, i) => {
...
/* opts: { tx, key } to edit an existing entry; { type } to start a new one */
function openEntry(opts) {
  const o = opts || {};
  const editing = !!o.tx;
  const src = o.tx || {};
  const srcKey = o.key || (src.d ? periodKeyFor(parseISO(src.d), state.config.startDay) : null);
  const firstOf = type => (state.plan.accounts.find(a => a.type === type) || {}).id;
  const st = {
    type: src.t || o.type || "exp",
    amount: src.a != null ? String(src.a) : "",
    note: src.n || "",
    cat: src.c || "",
    catTouched: !!src.c,
    acc: src.acc || state.config.lastAcc || state.plan.accounts[0].id,
    from: src.from || firstOf("bank") || state.plan.accounts[0].id,
    to: src.to || firstOf("wallet") || (state.plan.accounts[1] || state.plan.accounts[0]).id,
    date: src.d || iso(today())
  };
  let amtInput = null;

  const save = again => {
    const a = readAmount(st.amount);
    if (!(a > 0)) { flash(amtInput); amtInput.focus(); return; }
    if (st.type === "xfer" && st.from === st.to) { toast("Pick two different accounts to move money between."); return; }
    let entry;
    if (st.type === "xfer") {
      entry = { t: "xfer", a, from: st.from, to: st.to, d: st.date,
        n: st.note || (accById(st.from).name + " → " + accById(st.to).name) };
    } else if (st.type === "inc") {
      entry = { t: "inc", a, acc: st.acc, d: st.date, c: "", n: st.note || "Extra income" };
    } else {
      const cat = st.cat || guessCat(st.note) || FALLBACK_CAT;
      entry = { t: "exp", a, c: cat, acc: st.acc, d: st.date, n: st.note || catById(cat).name };
    }
    const key = periodKeyFor(parseISO(st.date), state.config.startDay);
    willChange((editing ? "change “" : "add “") + entry.n + "”");
    if (editing) {
      entry.id = src.id;
      const from = monthDoc(srcKey);
      const at = from.tx.findIndex(x => x.id === src.id);
      if (key === srcKey && at >= 0) from.tx[at] = entry;     // same period: fix in place
      else { from.tx = from.tx.filter(x => x.id !== src.id); monthDoc(key).tx.push(entry); }
      saveMonth(srcKey); if (key !== srcKey) saveMonth(key);
    } else {
      entry.id = uid();
      monthDoc(key).tx.push(entry);
      saveMonth(key);
    }
    if (st.type !== "xfer" && st.acc !== state.config.lastAcc) { state.config.lastAcc = st.acc; saveConfig(); }

    const what = (editing ? "Changed" : "Added") + " “" + entry.n + "” " + money(a);
    if (again) {                     // keep the sheet open for the next receipt
      st.amount = ""; st.note = ""; st.cat = ""; st.catTouched = false;
      renderAll(); done(what); drawBody(); amtInput.focus();
      return;
    }
    closeSheet();
    if (key !== state.period && !editing) { state.userMovedPeriod = true; state.period = key; watchPeriod(); }
    renderAll();
    done(what);
  };

  let drawBody = () => {};
  openSheet(editing ? "Change entry" : "Add", body => {
    drawBody = () => {
      body.textContent = "";
      const seg = chipRow([{ value: "exp", label: "Spent" }, { value: "inc", label: "Received" }, { value: "xfer", label: "Moved" }],
        st.type, v => { st.type = v; drawBody(); });
      seg.classList.add("seg");
      body.append(seg);

      const big = el("div", "amtbig");
      big.append(el("span", "cur", currencySymbol(state.config.currency)));
      amtInput = input("text", st.amount, { inputmode: "decimal", placeholder: "0", autocomplete: "off", "aria-label": "Amount" });
      amtInput.id = "entryAmt";
      amtInput.addEventListener("input", () => { st.amount = amtInput.value; });
      amtInput.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); save(false); } });
      big.append(amtInput);
      body.append(big);

      const accItems = state.plan.accounts.map(a => ({ value: a.id, label: a.name, hue: accHue(a.id) }));
      if (st.type === "xfer") {
        const f1 = el("div", "field"); f1.append(el("label", null, "From"), chipRow(accItems, st.from, v => { st.from = v; }));
        const f2 = el("div", "field"); f2.append(el("label", null, "To"), chipRow(accItems, st.to, v => { st.to = v; }));
        const help = el("p", "ehelp", "Charging PayPay or taking cash from an ATM moves your own money — it never counts as spending.");
        body.append(f1, f2, help);
      } else {
        const nf = el("div", "field");
        const note = input("text", st.note, { autocomplete: "off",
          placeholder: st.type === "inc" ? "Bonus, refund, side job…" : "Lawson, Daiso, lunch…" });
        note.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); save(false); } });
        nf.append(el("label", null, st.type === "inc" ? "From where" : "What for"), note);
        body.append(nf);

        if (st.type === "exp") {
          const cf = el("div", "field");
          const recent = recentCats(8);
          if (st.cat && !recent.includes(st.cat)) recent.unshift(st.cat);
          const items = recent.map(id => { const c = catById(id); return { value: id, label: c.name, hue: (GROUP[c.group] || GROUPS[2]).hue }; });
          const all = select([{ value: "", label: "Other category…" }].concat(catOptions()), recent.includes(st.cat) ? "" : st.cat);
          all.classList.add("chipsel");
          const chips = chipRow(items, st.cat, v => { st.cat = v; st.catTouched = true; all.value = ""; });
          all.addEventListener("change", () => { if (all.value) { st.cat = all.value; st.catTouched = true; chips._set(all.value); } });
          // typing a shop name picks its category, until you choose one yourself
          note.addEventListener("input", () => {
            st.note = note.value;
            if (st.catTouched) return;
            const g = guessCat(note.value);
            if (!g) return;
            st.cat = g;
            chips._set(g);
            all.value = recent.includes(g) ? "" : g;
          });
          cf.append(el("label", null, "Category"), chips, all);
          body.append(cf);
        } else {
          note.addEventListener("input", () => { st.note = note.value; });
        }
        const af = el("div", "field");
        af.append(el("label", null, st.type === "inc" ? "Into" : "Paid from"), chipRow(accItems, st.acc, v => { st.acc = v; }));
        body.append(af);
      }

      const t0 = iso(today()), yd = new Date(today()); yd.setDate(yd.getDate() - 1);
      const dateIn = input("date", st.date, { "aria-label": "Date" });
      dateIn.classList.add("chipdate");
      const dchips = chipRow([{ value: t0, label: "Today" }, { value: iso(yd), label: "Yesterday" }], st.date,
        v => { st.date = v; dateIn.value = v; });
      dateIn.addEventListener("change", () => { st.date = dateIn.value || t0; dchips._set(st.date); });
      dchips.append(dateIn);
      const df = el("div", "field"); df.append(el("label", null, "When"), dchips);
      body.append(df);

      if (editing) {
        const extras = el("div", "sheet-extras");
        if (st.type === "exp") {
          const dup = el("button", "btn btn-sm", "↻ Log again today");
          dup.addEventListener("click", () => { closeSheet(); repeatTx(src); });
          extras.append(dup);
        }
        const del = el("button", "btn btn-sm btn-danger", "Delete entry");
        del.addEventListener("click", () => { closeSheet(); deleteTx(srcKey, src.id); });
        extras.append(del);
        body.append(extras);
      }
    };
    ...
    ? [{ label: "Cancel", onClick: closeSheet }, { label: "Save changes", primary: true, onClick: () => save(false) }]
    : [{ label: "Add & another", onClick: () => save(true) }, { label: "Add", primary: true, onClick: () => save(false) }]);
  setTimeout(() => { if (amtInput && !editing) amtInput.focus(); }, 40);
}

/* ---------- fix a recurring bill for one period ----------
   The amount, whether it's paid, or skip it this month — without touching the
   bill itself, which stays as set up for every other month. */
function openBillMonth(billId, key) {
  const bill = (state.plan.fixed || []).find(b => b.id === billId);
  if (!bill) return;
  const m = state.months[key] || {};
  const a = billAmount(bill, key);
  const hasOverride = !!(m.billAmt && typeof m.billAmt[billId] === "number");
  const st = {
    amount: a.known ? String(a.v) : "",
    status: (m.skip || []).includes(billId) ? "skip" : (m.paid || []).includes(billId) ? "paid" : "due"
  };
  const c = catById(bill.cat);
  let amtIn = null;
  const save = () => {
    const typed = st.amount.trim() === "" ? null : readAmount(st.amount);
    willChange("change " + bill.name + " for " + periodShort(key, state.config.startDay));
    const md = monthDoc(key);
    if (typed == null) delete md.billAmt[billId];
    else if (!bill.variable && typed === num(bill.amount)) delete md.billAmt[billId];   // back to normal
    else md.billAmt[billId] = typed;
    md.paid = md.paid.filter(x => x !== billId);
    md.skip = md.skip.filter(x => x !== billId);
    if (st.status === "paid") md.paid.push(billId);
    if (st.status === "skip") md.skip.push(billId);
    saveMonth(key); closeSheet(); renderAll();
    done(bill.name + " updated for " + periodShort(key, state.config.startDay));
  };
  openSheet(bill.name + " — " + periodLabel(key, state.config.startDay), body => {
    const meta = el("p", "ehelp");
    meta.textContent = c.name + " · from " + accById(bill.acc).name + " · due on the " + ordinal(clamp(num(bill.day) || 1, 1, 28));
    body.append(meta);
    const big = el("div", "amtbig");
    big.append(el("span", "cur", currencySymbol(state.config.currency)));
    amtIn = input("text", st.amount, { inputmode: "decimal", autocomplete: "off", "aria-label": "Amount this month",
      placeholder: String(Math.round(a.v)) });
    amtIn.addEventListener("input", () => { st.amount = amtIn.value; });
    amtIn.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); save(); } });
    big.append(amtIn);
    body.append(big);
    const hint = el("p", "ehelp");
    hint.textContent = !a.known ? "Not entered yet — using an estimate of " + money(a.v) + " from your recent months."
      : hasOverride ? "Changed for this month only. Clear the box to go back to the usual " + money(num(bill.amount)) + "."
      : "Change this for this month only. The usual amount lives in Set up.";
    body.append(hint);
    const f = el("div", "field");
    f.append(el("label", null, "This month"), chipRow([
      { value: "due", label: "Not paid yet" }, { value: "paid", label: "Paid" }, { value: "skip", label: "Skip this month" }
    ], st.status, v => { st.status = v; }));
    body.append(f);
    body.append(el("p", "ehelp", "“Skip” removes it from this month entirely — for a month the gym was frozen, or rent was covered."));
  }, [{ label: "Cancel", onClick: closeSheet }, { label: "Save", primary: true, onClick: save }]);
  setTimeout(() => amtIn && amtIn.focus(), 40);
}

/* ---------- fix pay for one period ----------
   Overtime, a bonus, a short month: record what actually arrived, without
   changing your usual take-home in Set up. */
function openPayMonth(incId, key) {
  const inc = (state.plan.incomes || []).find(i => i.id === incId);
  if (!inc) return;
  const m = state.months[key] || {};
  const cur = incomeAmount(inc, key);
  const st = { amount: String(cur), status: (m.gotIncome || []).includes(incId) ? "got" : "due" };
  let amtIn = null;
  const save = () => {
    const v = st.amount.trim() === "" ? num(inc.amount) : readAmount(st.amount);
    willChange("change " + inc.name + " for " + periodShort(key, state.config.startDay));
    const md = monthDoc(key);
    if (v === num(inc.amount)) delete md.incomeAmt[incId]; else md.incomeAmt[incId] = v;
    md.gotIncome = md.gotIncome.filter(x => x !== incId);
    if (st.status === "got") md.gotIncome.push(incId);
    saveMonth(key); closeSheet(); renderAll();
    done(inc.name + " updated for " + periodShort(key, state.config.startDay));
  };
  openSheet(inc.name + " — " + periodLabel(key, state.config.startDay), body => {
    body.append(el("p", "ehelp", "Into " + accById(inc.acc).name + " · usually on the " + ordinal(clamp(num(inc.day) || 1, 1, 28))));
    const big = el("div", "amtbig");
    big.append(el("span", "cur", currencySymbol(state.config.currency)));
    amtIn = input("text", st.amount, { inputmode: "decimal", autocomplete: "off", "aria-label": "Amount received" });
    amtIn.addEventListener("input", () => { st.amount = amtIn.value; });
    amtIn.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); save(); } });
    big.append(amtIn);
    body.append(big);
    body.append(el("p", "ehelp", cur !== num(inc.amount)
      ? "Changed for this month only — usually " + money(num(inc.amount)) + "."
      : "Different this month? Type what actually arrived. Your usual amount stays as it is."));
    const f = el("div", "field");
    f.append(el("label", null, "This month"), chipRow([{ value: "due", label: "Not in yet" }, { value: "got", label: "Received" }],
      st.status, v => { st.status = v; }));
    body.append(f);
  }, [{ label: "Cancel", onClick: closeSheet }, { label: "Save", primary: true, onClick: save }]);
  setTimeout(() => { if (amtIn) { amtIn.focus(); amtIn.select(); } }, 40);
}

/* ---------- reset ----------
   Three sizes of "start again". Every one of them can be undone straight after,
   because it goes through the same storage layer as everything else. */
function openReset() {
  const allKeys = Object.keys(state.months);
  const count = keys => keys.reduce((s, k) => s + ((state.months[k] || {}).tx || []).length, 0);
  const thisN = (state.months[state.period] || {}).tx ? state.months[state.period].tx.length : 0;
  const allN = count(allKeys);
  const st = { which: "period" };
  const opts = {
    period: { label: "This period only", desc: "Removes the " + thisN + " entr" + (thisN === 1 ? "y" : "ies") + " in " + periodLabel(state.period, state.config.startDay) +
      ", and clears which bills and pay you ticked. Your accounts, bills and budgets stay.", btn: "Clear this period" },
    entries: { label: "Every entry", desc: "Removes all " + allN + " entr" + (allN === 1 ? "y" : "ies") + " across every period. Your accounts, pay, bills and budgets stay, so you can start logging again straight away.", btn: "Clear all entries" },
    all: { label: "Start over completely", desc: "Removes every entry and resets accounts, pay, bills, categories and budgets to a blank book. Your currency and period start are kept.", btn: "Start over" }
  };
  let desc = null, goBtn = null;
  const refresh = () => { desc.textContent = opts[st.which].desc; if (goBtn) goBtn.textContent = opts[st.which].btn; };
  const run = () => {
    willChange(opts[st.which].btn.toLowerCase());
    const blankMonth = k => ({ key: k, tx: [], skip: [], paid: [], gotIncome: [], billAmt: {}, incomeAmt: {} });
    if (st.which === "period") {
      state.months[state.period] = blankMonth(state.period); saveMonth(state.period);
    } else {
      allKeys.forEach(k => { state.months[k] = blankMonth(k); saveMonth(k); });
      if (st.which === "all") {
        const keep = { currency: state.config.currency, startDay: state.config.startDay, reservePct: state.config.reservePct };
        state.config = { ...blankConfig(), ...keep };
        state.plan = blankPlan(state.config.currency);
        migrate(); saveConfig(); savePlan();
      }
    }
    closeSheet(); renderAll();
    done(opts[st.which].btn + " — done");
  };
  openSheet("Reset", body => {
    const f = el("div", "field");
    f.append(el("label", null, "What to clear"), chipRow(
      Object.keys(opts).map(k => ({ value: k, label: opts[k].label })), st.which, v => { st.which = v; refresh(); }));
    body.append(f);
    desc = el("p", "reset-desc");
    body.append(desc);
    const safe = el("div", "sheet-extras");
    const exp = el("button", "btn btn-sm", "Export a backup first");
    exp.addEventListener("click", doExport);
    safe.append(exp, el("span", "ehelp", "You can also undo this straight after."));
    body.append(safe);
    refresh();
  }, [{ label: "Cancel", onClick: closeSheet }, { label: opts.period.btn, danger: true, onClick: run }]);
  goBtn = $("sheetFoot").lastElementChild;
  refresh();
}
...
function openReconcile() {
  openSheet("Correct a balance", body => {
    ...
    sheetSaver = () => {
      ...
      const dstr = iso(today());
      willChange("correct " + accById(acc.value).name + " balance");
      ...
      saveMonth(key); closeSheet(); renderAll();
      done(accById(acc.value).name + " balance corrected");
    };
...
function openAddBill() {
  openSheet("Add a recurring bill", body => {
    ...
    sheetSaver = () => {
      ...
      willChange("add bill “" + name.value.trim() + "”");
      ...
      done("Added bill “" + name.value.trim() + "”");
    };
    ...
    const drawMap = () => {
      ...
      const refresh = () => {
        ...
        sheetSaver = () => {
          ...
          willChange("import " + parsed.length + " rows");
          ...
          closeSheet(); renderAll();
          done("Imported " + parsed.length + " rows");
        };
      };
...
function buildCSV() {
  ...
  Object.keys(state.months).sort().forEach(k => {
    ...
    (state.plan.incomes || []).filter(i => got.has(i.id)).forEach(i => {
      rows.push([k, iso(dayInPeriod(i.day, k)), "salary (received)", "", "", i.name, incomeAmount(i, k), accById(i.acc).name, "", "", cur]);
    });
  });
...
const realThen = fn => () => { closeMenu(); ensureReal(); fn(); };
$("btnSetup").addEventListener("click", realThen(openSetup));
$("btnBudgets").addEventListener("click", realThen(openBudgets));
$("btnEditCats").addEventListener("click", realThen(openBudgets));
$("btnImport").addEventListener("click", realThen(openImport));
$("btnExport").addEventListener("click", () => { closeMenu(); doExport(); });
$("btnReset").addEventListener("click", () => { closeMenu(); if (state.sample) { ensureReal(); renderAll(); return; } openReset(); });
$("btnTransfer").addEventListener("click", realThen(() => openEntry({ type: "xfer" })));
$("btnReconcile").addEventListener("click", realThen(openReconcile));
$("btnAddBill").addEventListener("click", realThen(openAddBill));
$("btnAdd").addEventListener("click", realThen(() => openEntry()));
$("fab").addEventListener("click", realThen(() => openEntry()));
$("btnUndo").addEventListener("click", undo);
$("btnRedo").addEventListener("click", () => { closeMenu(); redo(); });
$("qaAdd").addEventListener("click", addQuick);
$("qaText").addEventListener("keydown", e => { if (e.key === "Enter") addQuick(); });

/* the ⋯ menu: opens on click, closes on a choice, an outside click or Escape */
function closeMenu() { $("menuPop").hidden = true; $("btnMore").setAttribute("aria-expanded", "false"); }
$("btnMore").addEventListener("click", e => {
  e.stopPropagation();
  const open = $("menuPop").hidden;
  $("menuPop").hidden = !open;
  $("btnMore").setAttribute("aria-expanded", open ? "true" : "false");
  if (open) { const first = $("menuPop").querySelector(".mi:not([disabled])"); if (first) first.focus(); }
});
document.addEventListener("click", e => { if (!$("menuPop").hidden && !e.target.closest(".menu")) closeMenu(); });

const typing = t => t && (t.tagName === "INPUT" || t.tagName === "SELECT" || t.tagName === "TEXTAREA" || t.isContentEditable);
document.addEventListener("keydown", e => {
  if (e.key === "Escape" && !$("menuPop").hidden) { closeMenu(); $("btnMore").focus(); return; }
  // Ctrl/Cmd+Z is the page's undo — except inside a text box, where it stays the box's own undo
  const mod = e.ctrlKey || e.metaKey;
  if (mod && !typing(e.target) && $("scrim").hidden) {
    const k = e.key.toLowerCase();
    if (k === "z" && !e.shiftKey) { e.preventDefault(); undo(); return; }
    if ((k === "z" && e.shiftKey) || k === "y") { e.preventDefault(); redo(); return; }
  }
  if (mod || e.altKey || typing(e.target) || !$("scrim").hidden) return;
  if (e.key === "n") { e.preventDefault(); ensureReal(); openEntry(); }
  else if (e.key === "/") { e.preventDefault(); $("lSearch").focus(); $("lSearch").scrollIntoView({ block: "center", behavior: "smooth" }); }
});

function setScope(s) {
  state.scope = s;
  $("scopePeriod").classList.toggle("on", s === "period"); $("scopePeriod").setAttribute("aria-pressed", s === "period");
  $("scopeAll").classList.toggle("on", s === "all");       $("scopeAll").setAttribute("aria-pressed", s === "all");
  renderLedger(derive(state.period));
}
$("scopePeriod").addEventListener("click", () => setScope("period"));
$("scopeAll").addEventListener("click", () => { setScope("all"); $("lSearch").focus(); });
...
function watchPeriod() {
  ...
  unsubActive = state.db.doc("months/" + key).onSnapshot(snap => {
    ...
    state.months[key] = monthFrom(key, snap.data());
    if (state.ready) { migrate(); markSaved("months/" + key, state.months[key]); renderAll(); }
  }, err => console.warn("month watch", err.code));
}

/* everything just loaded becomes the baseline undo compares against */
function markAllSaved() {
  markSaved("config/main", state.config);
  markSaved("plan/main", state.plan);
  Object.keys(state.months).forEach(k => markSaved("months/" + k, state.months[k]));
}
  ...
  useCap("db").then(db => {
    ...
    db.doc("config/main").onSnapshot(snap => {
      if (snap.exists) {
        ...
        migrate(); markSaved("config/main", state.config); watchPeriod(); renderAll();
      } else if (!sawConfig) {
        ...
        if (lc && lp) {
          ...
          state.sample = false;
          migrate();
          // copying a local book up is not a user change, so it is not undoable
          restoring = true;
          saveConfig(); savePlan();
    ...
    db.doc("plan/main").onSnapshot(snap => {
      ...
      state.sample = false;
      migrate(); markSaved("plan/main", state.plan); renderAll();
    }, err => console.warn("plan watch", err.code));

    db.collection("months").orderBy("key", "desc").limit(24).onSnapshot(snap => {
      if (!snap.docs.length) return;
      snap.docs.forEach(doc => { state.months[doc.id] = monthFrom(doc.id, doc.data()); });
      state.sample = false;
      migrate();
      snap.docs.forEach(doc => markSaved("months/" + doc.id, state.months[doc.id]));
      renderAll();
